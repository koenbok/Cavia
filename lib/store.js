(function() {
  var DeleteQuery, SelectQuery, Store, async, inspect, log, utils, _,
  ...
  Store = (function() {
    ...
    Store.prototype.get = function(kind, keys, callback) {
      ...
      return this.backend.query(query, function(err, results) {
        var item, key, resultData, resultMap, resultMapped, _i, _j, _len, _len2;
        if (single) {
          if (results[0]) {
    ...
    Store.prototype.query = function(kind, filter, options, callback) {
      ...
      if (callback == null) callback = filter;
